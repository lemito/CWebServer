  ...
  if (req_method == GET) {
    ...
    if (body) {
      ...
      if (name_start) {
        ...
        snprintf(greeting_msg, sizeof(greeting_msg),
                 "<p>Hello %s</p><br><a href='/'>Back to home</a>", name);
...
void handle_404(int sockfd) {
  char *message = malloc(strlen("<h1>404</h1>") +
                         strlen(text_creator("Page not found")) + 1);
  if (message == NULL) {
...
void *handle_client_thread(void *arg) {
  ...
  snprintf(log_msg, 256, "Кто-то подключился на url %s, номер сокета = %d", url,
           new_sockfd);
  log_write(log_msg);
...
void router(char *route, int sockfd, char *buffer) {
  if (strcmp(route, "/") == 0) {
    ...
    if (method(buffer) == GET)
      ...
      char *response = response_creator(HTTP_NOT_ALLOWED, "text/plain",
                                        "Метод запрещен для данного пути");
      write_response(sockfd, response, strlen(response));
      FREE_AND_NULL(response);
    }
  } else if (strcmp(route, "/favicon.ico") == 0) {
    char *response =
        response_creator(HTTP_NOT_FOUND, "text/plain", "Favicon not available");
    write_response(sockfd, response, strlen(response));
    FREE_AND_NULL(response);
